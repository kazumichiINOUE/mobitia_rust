...
use crate::app::OsmoState;

#[derive(Default)]
pub struct OsmoScreen {}
...
impl OsmoScreen {
    ...
    pub fn draw(&mut self, ui: &mut egui::Ui, osmo_state: &OsmoState) {
        ui.heading("Osmo Mode");
        if let Some(texture) = &osmo_state.texture {
            // 利用可能な描画領域のサイズを取得
            ...
            ui.centered_and_justified(|ui| {
                ui.label(&osmo_state.connection_status);
            });
        }
    }
}
