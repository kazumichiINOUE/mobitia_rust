        ...
        Commands::Save { command } => match command {
            SaveCommands::Image => {
                ...
                let save_dir = match dirs::download_dir() {
                    ...
                    None => {
                        sender_clone.send("ERROR: Could not find download directory.".to_string()).unwrap_or_default();
                        return;
                    }
                };
                if let Err(e) = std::fs::create_dir_all(&save_dir) {
                    sender_clone.send(format!("ERROR: Failed to create directory '{}': {}", save_dir.display(), e)).unwrap_or_default();
                    return;
                }
                ...
                thread::spawn(move || {
                    ...
                    match img.save(&save_path) {
                        Ok(_) => sender_clone.send(format!("Image saved to: {}", save_path.display())).unwrap_or_default(),
            ...
            SaveCommands::Points { output } => {
                let path_to_save = if let Some(path_str) = output {
                    ...
                    let save_dir = match dirs::download_dir() {
                        Some(path) => path.join(Local::now().format("%Y-%m-%d").to_string()),
                        None => {
                             app.command_history.push(ConsoleOutputEntry { text: "ERROR: Could not find download directory.".to_string(), group_id: current_group_id });
                            return;
                        }
                    };
                    if let Err(e) = std::fs::create_dir_all(&save_dir) {
                        app.command_history.push(ConsoleOutputEntry { text: format!("ERROR: Failed to create directory '{}': {}", save_dir.display(), e), group_id: current_group_id });
                        return;
                    }
                    let timestamp = Local::now().format("%H%M%S").to_string();
                ...
                app.command_history.push(ConsoleOutputEntry { text: format!("Requesting save of point cloud to '{}'...", path_to_save), group_id: current_group_id });
